...
/*
 * Auxiliary estimator used only for logging.
 * It receives the same measurements as coreData, but its process covariance is
 * inflated during every update so the logged state relies much less on the
 * dynamics prediction. This state is NOT externalized to the controller.
 */
...
/* Implemented in kalman_core.c. Used only by coreData_wodynamics. */
void kalmanCoreAddProcessNoiseNoDynamics(kalmanCoreData_t *this,
...
static void kalmanTask(void* parameters) {
  ...
  while (true) {
    ...
    if (nowMs >= nextPredictionMs) {
      ...
      kalmanCorePredict(&coreData_wodynamics, &coreParams_wodynamics, &accSubSampler.subSample, &gyroSubSampler.subSample, nowMs, quadIsFlying);
    ...
    kalmanCoreAddProcessNoiseNoDynamics(&coreData_wodynamics, &coreParams_wodynamics, nowMs);
...
void estimatorKalmanInit(void)
{
  ...
  coreParams_wodynamics.attitudeReversion = coreParams.attitudeReversion;
 ...
 */
 ...
 /**
 * @brief Auxiliary position x/y with dynamics weakened for the entire flight.
 *
 * These variables are for logging/debug only. They are not used by the controller.
 */
  LOG_ADD(LOG_FLOAT, stateX_noDyn, &coreData_wodynamics.S[KC_STATE_X])
  LOG_ADD(LOG_FLOAT, stateY_noDyn, &coreData_wodynamics.S[KC_STATE_Y])
 /**
 ...
 */
  ...
  */
  ...
  LOG_ADD(LOG_FLOAT, statePX_noDyn, &coreData_wodynamics.S[KC_STATE_PX])
  LOG_ADD(LOG_FLOAT, statePY_noDyn, &coreData_wodynamics.S[KC_STATE_PY])
  /**
  ...
  */
  ...
  LOG_ADD(LOG_FLOAT, statePZ_noDyn, &coreData_wodynamics.S[KC_STATE_PZ])
  /**
