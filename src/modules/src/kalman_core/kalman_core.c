...
void kalmanCoreAddProcessNoiseNoDynamics(kalmanCoreData_t *this, const kalmanCoreParams_t *params, const uint32_t nowMs) {
  float dt = (nowMs - this->lastProcessNoiseUpdateMs) / 1000.0f;
  if (dt > 0.0f) {
    addProcessNoiseDt(this, params, dt);

    /*
     * Logging-only no/weak-dynamics mode.
     *
     * Keep the normal prediction and measurement updates, but make the
     * X/Y/PX/PY prediction covariance very large at every update. This makes
     * the auxiliary estimator trust the incoming measurements much more than
     * the dynamics prediction during the whole flight.
     */
    const float extraNoise = 1000.0f;

    this->P[KC_STATE_X][KC_STATE_X] += extraNoise;
    this->P[KC_STATE_Y][KC_STATE_Y] += extraNoise;
    this->P[KC_STATE_PX][KC_STATE_PX] += extraNoise;
    this->P[KC_STATE_PY][KC_STATE_PY] += extraNoise;
